...
void pageRank(Graph g, double *solution, double damping, double convergence) {
    ...
    */
    ...
    while (!converged && iter < MAXITER) {
        ...
        #pragma omp parallel
        {
            double priv_score_new[num_nodes];

            
            for (int i = 0; i < numNodes; ++i) {
                priv_score_new[i] = 0.0;
                ...
                for (const Vertex *v = in_begin; v < in_end; ++v) {
                    priv_score_new[i] += score_old[*v] / outgoing_size(g, *v);
                }
                priv_score_new[i] =
                    damping * priv_score_new[i] + (1.0 - damping) * equal_prob;
            }

            #pragma omp critical
            for (int i = 0; i < numNodes; ++i) {
                priv_score_new[i] += damping * broadcastScore * equal_prob;
                globalDiff += std::abs(priv_score_new[i] - score_old[i]);
            }
            converged = (globalDiff < convergence);
            std::swap(priv_score_new, score_old);
        }
    }
